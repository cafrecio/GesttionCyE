        ...
        <div class="content-wrapper">
            <div class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1 class="m-0">Editar Localidad</h1>
                    ...
                    <div class="col-md-6">
                        <div class="card card-primary">
                            <div class="card-header">
                                <h3 class="card-title">Formulario de Edición de Localidad</h3>
                            </div>
                            @if(isset($localidad))
                                <p>ID de la localidad dentro de edit.blade.php: {{ $localidad->id }}</p>
                                <p>Nombre de la localidad dentro de edit.blade.php: {{ $localidad->nombre }}</p>
                            @else
                                <p>¡¡¡ERROR!!! La variable $localidad NO está definida en edit.blade.php</p>
                            @endif
                            <form action="{{ route('localidades.update', $localidad->id) }}" method="POST">
                                ...
                                <div class="card-body">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="nombre" name="nombre" value="{{ $localidad->nombre }}" placeholder="Ingrese el nombre">
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="provincia_id" name="provincia_id" required>
                                            ...
                                            @foreach ($provincias as $provincia)
                                                <option value="{{ $provincia->id }}" {{ $localidad->provincia_id == $provincia->id ? 'selected' : '' }}>{{ $provincia->nombre }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="zona_id" name="zona_id" required>
                                            ...
                                            @foreach ($zonas as $zona)
                                                <option value="{{ $zona->id }}" {{ $localidad->zona_id == $zona->id ? 'selected' : '' }}>{{ $zona->nombre }}</option>
                                            @endforeach
                                        </select>
                            ...
                            <form action="{{ route('localidades.destroy', $localidad->id) }}" method="POST" style="display: inline; margin-top: 20px;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger float-right">Eliminar Localidad</button>
                            </form>
                        </div>
                        </div>
                    </div>
